...
7. 提交前运行 `python scripts/check_sensitive.py --staged` 和 `npm run security:validate`；维护者首次 clone 后运行 `npm run security:install-hook`；只暂存明确文件，禁止 `git add .` / `git add -A`。
